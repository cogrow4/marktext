...
const cli = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to NovelCraft and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'novelcraft-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: novelcraft [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`NovelCraft: ${global.NOVELCRAFT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'novelcraft-user-data')
