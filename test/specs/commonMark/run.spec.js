...
export const writeResult = (version, specs, markedSpecs, type = 'commonmark') => {
  ...
  compareResult += `NovelCraft failed examples count: ${failedCount}\n\n`
  let count = 0
  specs.forEach((spec, i) => {
    if (spec.shouldFail !== markedSpecs[i].shouldFail) {
      ...
      compareResult += `NovelCraft ${spec.shouldFail ? 'fail' : 'success'} and marked.js ${markedSpecs[i].shouldFail ? 'fail' : 'success'}\n\n`
