...
<h2 align="center">支援 NovelCraft</h2>

NovelCraft 是基於 MIT 授權條款的開源專案，您可以持續在 GitHub 發布頁面免費下載最新版本。NovelCraft 仍然在開發中，其發展與所有贊助商都密不可分。我希望您加入此行列：

- [透過 Patreon 成為支持者或贊助商](https://www.patreon.com/ranluo) 或 [一次性贊助](https://github.com/Jocs/sponsor.me)
- [透過 Open Collective 成為支持者或贊助商](https://opencollective.com/novelcraft)

##### 使用 Patreon 和 OpenCollective 贊助有什麼不同？

使用 Patreon 贊助，它將直接贊助给 NovelCraft 的作者及維護者 Luo Ran (@jocs)。使用 Open Collective 贊助的所有費用都是公開的，這些贊助資金將用於 NovelCraft 的開發、維護、線上和線下活動以及一些必要的資源（無論您是在 Patreon 還是 Open Collective 贊助）。您的姓名或公司logo將出現在 NovelCraft 的 README 和官方網站上。
...
**NovelCraft** 使用 virtual DOM 來呈現畫面，擁有高效能和開源的附加優勢。任何喜歡 Markdown 和寫作的人都可以使用 NovelCraft。
2. 承上所述，**NovelCraft** 是完全免費和開源的，且將永遠開源。我們希望所有 Markdown 愛好者都可以協助開發並貢獻自己的程式，讓 **NovelCraft** 成為流行的 Markdown 編輯器。
3. Markdown 編輯器很多，各有不同的優點與功能。我們很難滿足所有 Markdown 使用者的需求，但是我們希望 **NovelCraft** 盡可能滿足每一個 Markdown 使用者的需求。儘管最新的 **NovelCraft** 仍不完美，但我們會盡力使它更加完善。
...
您可以從 [release page](https://github.com/novelcraft/novelcraft/releases/latest) 下載最新的 `novelcraft-%version%.dmg` 或是使用 [**homebrew cask**](https://github.com/caskroom/homebrew-cask) 安裝 NovelCraft。如果使用 Homebrew-Cask，您只需要安裝 [Homebrew](https://brew.sh/)。
...
只需要透過安裝精靈 (`novelcraft-setup-％version％.exe`）下載並安裝 NovelCraft，然後選擇為單一使用者或是所有使用者安裝。

或者可以使用 Chocolatey 或 Winget 等套件管理器來安裝 NovelCraft。
...
您可以從以下網址下載 Linux、macOS 和 Windows 的安裝檔：[release page](https://github.com/novelcraft/novelcraft/releases/latest)。如果該版本在您的系統中無法正常使用，請開一個 [issue](https://github.com/novelcraft/novelcraft/issues)。

## 開發

如果您想自己建置 **NovelCraft**，請查看我們的 [build instructions](../../docs/dev/BUILD.md)。

- [使用者說明文件](../../docs/README.md)
- [開發人員說明文件](../../docs/dev/README.md)

如果您對 **NovelCraft** 有任何疑問，歡迎開一個 issue，並使用開 issue 時的預設格式。當然您也可以直接提交 PR，我們將感激不盡。

## 整合
- [Alfred Workflow](http://www.packal.org/workflow/mark-text)：macOS 應用程式 Alfred 的使用流程：使用 "mt" 在檔案或者檔案夾中打開 NovelCraft。

## 貢獻

NovelCraft 正在全面開發中，請確保在提出 PR 之前先閱讀 [Contributing Guide](../../CONTRIBUTING.md) 想要為 NovelCraft 新增一些功能嗎？請先看看 [roadmap](../../ROADMAP.md) 並新增一個 issue。

## 貢獻者

感謝所有為 NovelCraft 做出貢獻的人們
[[contributors](https://github.com/novelcraft/novelcraft/graphs/contributors)]

特别感謝 @[Yasujizr](https://github.com/Yasujizr) 設計了一個 Logo 給 NovelCraft。
