...
<h2 align="center">支持 NovelCraft</h2>

NovelCraft 是一个使用 MIT license 的开源项目，您将一直能够从 GitHub release 页面免费下载最新版本。NovelCraft 仍然在开发中，它的发展离不开所有赞助者，希望您能加入他们的行列：

- [在 Patreon 上成为支持者或赞助者](https://www.patreon.com/ranluo) 或 [一次性捐赠](https://github.com/Jocs/sponsor.me)
- [在 Open Collective 上成为支持者或赞助者](https://opencollective.com/novelcraft)

##### Patreon 和 OpenCollective 有什么不同？

在 Patreon 赞助：资金将直接赞助给创建并继续维护 NovelCraft 的 Luo Ran (@jocs)。
在 Open Collective 赞助：所有费用都是透明的，这些赞助资金将用于 NovelCraft 的开发、维护、在线和离线活动以及一些必要的资源。
所有赞助者（无论是在 Patreon 还是 Open Collective）的姓名或公司徽标将出现在 NovelCraft 的官方网站和 README.md 文件中。
...
1. 我爱写作。我曾经使用过很多 Markdown 编辑器，但还没有一个编辑器可以完全满足我的需求。我不喜欢当我写一些难以忍受的错误时会被打扰。**NovelCraft** 使用 virtual DOM 来渲染页面，具有高效和开源的附加优势。这样，任何喜欢 Markdown 和写作的人都可以使用 NovelCraft。
2. 如上所述，**NovelCraft** 是完全自由开源的，并且将永远是开源的。我们希望所有 Markdown 爱好者都可以贡献自己的代码，并帮助将 **NovelCraft** 开发为流行的 Markdown 编辑器。
3. Markdown 编辑器很多，各有优点，有一些拥有独特的特性。我们很难满足每个 Markdown 用户的需求，但是我们希望 **NovelCraft** 能够尽可能满足每个 Markdown 用户的需求。尽管最新的 **NovelCraft** 仍不完美，但我们将尽力使它尽可能地完善。
...
您可以从 [release 页面](https://github.com/novelcraft/novelcraft/releases/latest)下载最新的 `novelcraft-%version%.dmg` 或者使用 [**homebrew cask**](https://github.com/caskroom/homebrew-cask) 安装 NovelCraft。要使用 Homebrew-Cask，您只需要安装 [Homebrew](https://brew.sh/)。
...
要想安装 NovelCraft，只需下载并运行安装向导（`novelcraft-setup-％version％.exe`），然后选择为本用户安装还是为本计算机所有用户安装。

或者，也可以使用 Chocolatey 或 Winget 等软件包管理器来安装 NovelCraft。
...
您可以从 [release 页面](https://github.com/novelcraft/novelcraft/releases/latest)下载适用于 Linux、macOS 和 Windows 的所有二进制文件。如果您的系统无法正常使用某个版本，请创建一个 [issue](https://github.com/novelcraft/novelcraft/issues)。

## 开发

如果您想自己构建 **NovelCraft**，请查看我们的[构建指南](../../docs/dev/BUILD.md)。

- [用户文档](../../docs/README.md)
- [开发者文档](../../docs/dev/README.md)

如果您对 **NovelCraft** 有任何疑问，欢迎写一个 issue。当这样做时，请使用打开 issue 时的默认格式。当然，如果您直接提交 PR，我们将不胜感激。

## 集成

- [Alfred 工作流](http://www.packal.org/workflow/mark-text)：macOS 应用程序 Alfred 的工作流：使用“mt”在文件或者文件夹下打开 NovelCraft。

## 贡献

NovelCraft 正在全面开发中，请确保在提出 PR 之前先阅读[贡献指南](../../CONTRIBUTING.md)。想要给 NovelCraft 添加一些功能？请先看看 [roadmap](../../ROADMAP.md) 和开放的 issue。

## 贡献者

感谢所有为 NovelCraft 做出贡献的人[[贡献者](https://github.com/novelcraft/novelcraft/graphs/contributors)]

特别感谢设计了 NovelCraft 图标的 @[Yasujizr](https://github.com/Yasujizr)。
