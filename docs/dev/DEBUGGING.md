...
The most simplest way is to debug using the `Debug NovelCraft` configuration. You can set breakpoints and use the `debugger` statement.
...
After the successful launch of NovelCraft, press `Ctrl+C` on both `node-profiler` instances. The tools created two files named `main.cpuprofile` and `renderer.cpuprofile`. You can now analyse these files via *Chrome Developer Tools* or *Visual Studio Code*.
